...
def sign_in_with_email_and_password(email, password):
    request_ref = f"https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword?key={st.secrets['FIREBASE_WEB_API_KEY']}"
...
def get_account_info(id_token):
    request_ref = f"https://www.googleapis.com/identitytoolkit/v3/relyingparty/getAccountInfo?key={st.secrets['FIREBASE_WEB_API_KEY']}"
    headers = {"content-type": "application/json; charset=UTF-8"}
...
def send_email_verification(id_token):
    request_ref = f"https://www.googleapis.com/identitytoolkit/v3/relyingparty/getOobConfirmationCode?key={st.secrets['FIREBASE_WEB_API_KEY']}"
    headers = {"content-type": "application/json; charset=UTF-8"}
...
def create_user_with_email_and_password(username, email, password):
    request_ref = f"https://www.googleapis.com/identitytoolkit/v3/relyingparty/signupNewUser?key={st.secrets['FIREBASE_WEB_API_KEY']}"
    headers = {"content-type": "application/json; charset=UTF-8" }
    data = json.dumps({"email": email, "password": password, "displayName":username, "returnSecureToken": True})
    request_object = requests.post(request_ref, headers=headers, data=data)
    raise_detailed_error(request_object)
    return request_object.json()

def delete_user_account(id_token):
    request_ref = f"https://www.googleapis.com/identitytoolkit/v3/relyingparty/deleteAccount?key={st.secrets['FIREBASE_WEB_API_KEY']}"
    headers = {"content-type": "application/json; charset=UTF-8"}
    data = json.dumps({"idToken": id_token})
    request_object = requests.post(request_ref, headers=headers, data=data)
    raise_detailed_error(request_object)
...
def sign_in(email:str, password:str) -> None:
    try:
        ...
        if error_message in {"INVALID_EMAIL","EMAIL_NOT_FOUND","INVALID_PASSWORD","MISSING_PASSWORD"}:
            st.session_state.auth_warning = 'Invalid e-mail or password'
        else:
            st.session_state.auth_warning = 'Invalid e-mail or password'
...
def create_account(username:str, email:str, password:str) -> None:
    try:
        ...
        send_email_verification(id_token)
        st.session_state.auth_success = 'Check your inbox to verify your email. Be sure to check the spam'
